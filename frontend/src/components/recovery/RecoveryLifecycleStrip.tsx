...
export function RecoveryLifecycleStrip({
  ...
  const steps = [
    {
      ...
      description: 'Raw payment.failed event received with error codes & rail telemetry.',
      icon: AlertTriangle,
    ...
    },
    {
      ...
      description: 'AI planner & deterministic rules classify failure into 1 of 6 bounded categories.',
      icon: BrainCircuit,
    ...
    },
    {
      ...
      description: 'Touch limits & margin checked, then smart link, debit retry, or WhatsApp sent.',
      icon: Send,
    ...
    },
    {
      ...
      description: 'Payment captured on gateway; recovery measured against 10% uncontacted holdout arm.',
      icon: CheckCircle2,
  ...
  return (
    <div className="rounded-panel border border-border bg-surface-sunken/40 p-4 space-y-3">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 border-b border-border/40 pb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-ink-muted hidden md:inline">
            How failed transactions progress through diagnosis, policy guardrails, and resolution
          </span>
        </div>
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div
              ...
              className="p-3 rounded-control border border-border/80 bg-surface flex flex-col justify-between space-y-2 hover:border-accent/60 transition-colors cursor-pointer group"
            >
              ...
              <div className="pt-1.5 border-t border-border/40 flex items-center justify-between font-mono text-[10px]">
