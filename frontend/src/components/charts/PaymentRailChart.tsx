      ...
      <CardContent className="space-y-4 font-mono text-xs">
        {railPerformance.length === 0 ? (
          ...
          railPerformance.map((item) => (
            <div key={item.rail} className="space-y-1.5 p-3 rounded-control bg-surface-sunken/60 border border-border">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-ink uppercase">{item.rail.replace('_', ' ')}</span>
                  <span className="text-[10px] text-ink-muted">({item.total_cases.toString()} cases)</span>
                </div>
              ...
              <div className="h-2 w-full rounded-full bg-surface-sunken overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, item.recovery_rate_pct).toString()}%` }}
                />
              </div>
              ...
              <div className="flex items-center justify-between text-[10px] text-ink-subtle pt-0.5">
                ...
                <span className="text-ink font-semibold">Recovered: {formatINR(item.recovered_paise)}</span>
              </div>
            </div>
          ))
        )}
