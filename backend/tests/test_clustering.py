...
def make_case(index: int, customer_id: str = "customer") -> RecoveryCase:
    ...
    return RecoveryCase(
        case_id=f"case-{index}", amount_paise=10000 + index * 100,
        currency="INR", experiment_arm=ExperimentArm.TREATMENT,
        ...
        failure_event=RawFailureEvent(event_id=f"event-{index}", payment_id=f"payment-{index}", customer_id=customer_id, amount_paise=10000 + index * 100, currency="INR", payment_rail=PaymentRail.UPI, error_code="U30", error_reason="funds", occurred_at=occurred_at),
    )
...
def test_clustering_is_deterministic_and_persists(tmp_path: Path) -> None:
    ...
    assert [{key: value for key, value in item.items() if key not in {"alert_id", "run_id", "created_at"}} for item in first] == [{key: value for key, value in item.items() if key not in {"alert_id", "run_id", "created_at"}} for item in second]
    persisted = repo.list_pattern_alerts()
    assert all(item["dominant_category"] == FailureCategory.LIQUIDITY_CONSTRAINT.value for item in persisted)
    assert {item["dominant_intervention"] for item in persisted} == {"SMART_RETRY", "NO_ACTION"}


def test_clustering_handles_empty_single_and_same_customer_without_mutation(tmp_path: Path) -> None:
    repo = CaseRepository(storage_path=tmp_path / "patterns.db")
